# Lab 5 - Starter
Make sure you make a PR to your own repo's main and not the class' repo!! Otherwise you will lose points!!

Jonathan Le

1. I would not use a unit test to test the "message" feature of a messaging application beacuse that feature involves many individual components that interacts with each other to perform that onne feature. As a result, it wouldn't make sense to test that entire process with a unit test.
   
2. I would use a unit test to test the "max message length" feature of a messaging application because that it is one individual component that can be tested easily. The threshold can be easily tested with cases below 80, exactly 80, over 80, and empty to check for all possible cases of that unit.